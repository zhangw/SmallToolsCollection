...
#TODO:maybe we need to configure the logger,otherwise it won't work.
...
if platform.system() != 'Windows':
	...
	dist_path = os.path.split(os.path.dirname(__file__))[0]
	template_dir = join(dist_path,'template')
...
def opendocx(file):
    ...
    #TODO:not only read the document.xml,but also extract other files for
    #regenerating the new docx correctly?
    mydoc.extractall(path=template_dir)
...
def savedocx(document, coreprops, appprops, contenttypes, websettings, wordrelationships, output):
    ...
    # I have hacked the treesandfiles,because I don't wanna to 
    # modify other files except 'word/document.xml'.
...
#                     wordrelationships: 'word/_rels/document.xml.rels'}
    ...
    #TODO:In my case,I don't implement the 'save as' method now.
    # Add & compress support files
    # nuisance from some os's and ignore the git file in my project
    ...
    for dirpath, dirnames, filenames in os.walk('.'):
        for filename in filenames:
            ...
            #make sure that the other original files reserved.
            if archivename not in treesandfiles.values():
